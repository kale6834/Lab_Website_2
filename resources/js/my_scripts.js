...
function loadStatsPage() {
    ...
    for (index = 2; index < table.rows.length; index++) {
        var homeScore = table.rows[index].cells[2].innerHTML;
        var oppScore = table.rows[index].cells[3].innerHTML;
        if (homeScore > oppScore) {
            winner = "CU Boulder";
            numWins++;
        } else if (oppScore > homeScore) {
            winner = table.rows[index].cells[1].innerHTML;
            numLosses++;
        } else {
            winner = "Tie";
        }
        table.rows[index].cells[4].innerHTML = winner;
    }
...
function loadPlayersPage() {
    ...
    for (var i = 0; i < players.length; i++) {
        tag += '<a href="#" style="display: block;" onclick="switchPlayers(' + i + ')">' + players[i].name + '</a>';
    }
